...
import com.alibaba.android.rimet.imiracle.Gps
...
//        val provider = mLocationManager!!.getBestProvider(criteria, true)
        ...
        Log.i("tag", "provider === >"+provider+", location---->"+location.toString())
        if (location != null) {
            Log.i("tag", "latitude = "+location.latitude+", longitude = "+location.longitude+", time = "+location.time)
        ...
        for(providerItem in providers){
            Log.i("tag", "register lister ------>"+providerItem)
